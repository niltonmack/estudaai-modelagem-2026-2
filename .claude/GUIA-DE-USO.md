# Guia de uso — Pipeline de 7 Skills (Claude Code)

Este guia é o equivalente Claude Code de `.cursor/rules/GUIA-DE-USO.md`.
O conteúdo dos 7 papéis é o mesmo; mudam a pasta e a forma de ativação.

No Cursor esses papéis ficam em `.cursor/rules/*.mdc` com `alwaysApply: false`.
No Claude Code o equivalente é **skill** (`.claude/skills/<nome>/SKILL.md`), porque o corpo só carrega quando a tarefa combina ou quando você chama `/nome`.

---

## Instalação

1. Use a pasta `.claude/skills/` (já existe neste repositório-modelo).
2. Cada skill é um diretório com `SKILL.md`.
3. Crie também a pasta `context/` na raiz. É onde cada skill salva o artefato para a próxima.
...
│   ├── rules/
│   └── skills/
│       ├── 01-requirements-analyst/SKILL.md
│       ├── 02-senior-stack-architect/SKILL.md
│       ├── 03-database-designer/SKILL.md
│       ├── 04-ui-ux-designer/SKILL.md
│       ├── 05-implementation-engineer/SKILL.md
│       ├── 06-qa-engineer/SKILL.md
│       ├── 07-docs-and-ops/SKILL.md
...
## Como as skills se ativam no Claude Code

O Claude lê `name` + `description` no início da sessão. O corpo do `SKILL.md` só entra quando:

- a sua mensagem casa com a `description`, ou
- você chama explicitamente `/01-requirements-analyst` (o nome da pasta).
...
Se o Claude Code não ativar a skill automaticamente, force a chamada pelo nome:
